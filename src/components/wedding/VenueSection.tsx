  ...
  return (
    <section id="venue" ref={ref} className="relative py-20 md:py-36 overflow-hidden" style={{ backgroundColor: "transparent" }}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7 }} className="text-center mb-10 md:mb-14 px-6">
        <p className="text-xs md:text-sm tracking-[0.25em] uppercase mb-3 font-body" style={{ color: "hsl(var(--wedding-gold))" }}>Join us for the evening</p>
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 space-y-8">
        ...
        >
          <img
            src="https://drive.google.com/thumbnail?id=1kHPHCODpG4c1Yh9bR7229C9FLAybiT0k&sz=w1920"
            alt="Mayflower Farmhouse"
            style={{ height: '200px' }}
        ...
        <div className="max-w-lg mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.2 }}
            ...
            <h3 className="font-display text-xl md:text-2xl mb-3" style={{ color: "hsl(var(--wedding-green))" }}>The Reception</h3>
            <p className="font-sub text-base" style={{ color: "hsl(var(--wedding-green))" }}>Mayflower Farmhouse, Bangalore</p>
            <p className="font-body text-xs mt-2" style={{ color: "#999" }}>Large Lawn · Round Table Setup</p>
            <p className="font-body text-xs mt-1 italic" style={{ color: "hsl(var(--wedding-gold))" }}>Stage for Sangeet · Decor lighting focus</p>
            <p className="font-body text-sm mt-3 font-medium" style={{ color: "hsl(var(--wedding-green))" }}>5:00 PM Onwards</p>
          </motion.div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 50, scale: 0.97 }} animate={isInView ? { opacity: 1, y: 0, scale: 1 } : {}} transition={{ duration: 1, delay: 0.4, ease: [0.22, 1, 0.36, 1] }}
          ...
          <iframe src="https://maps.google.com/maps?q=Mayflower+Farmhouse+Bengaluru&output=embed" className="w-full h-[250px] md:h-[350px]" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Mayflower Farmhouse Location" />
          <div className="p-5 md:p-8 bg-white relative">
            ...
            <motion.p className="font-body text-xs md:text-sm leading-relaxed mb-5 md:mb-6 max-w-2xl" style={{ color: "#999" }} initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 0.6, delay: 0.6 }}>
              A beautiful farmhouse venue surrounded by nature, perfect for an intimate garden wedding celebration.
            </motion.p>
            <motion.div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-4" initial={{ opacity: 0, y: 15 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.7 }}>
              <motion.a whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }}
                href="https://maps.app.goo.gl/od5DdXexHbDEmaBN7" target="_blank" rel="noopener noreferrer"
